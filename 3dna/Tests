...
class Tests : 
    ...
    class GA:
        """
        Classe simulant le contexte d'exécution : 
        - 'population_size' pour contrôler la taille des populations 
        - 'crossover' pour générer un enfant à partir de deux parents
        - 'create_new_gen' qui est la fonction à tester.
        """
        ...
        def crossover(self, parent1, parent2, crossover_type):
            """
            Ex. minimal : crée un enfant dont la fitness est la moyenne 
            des fitness des deux parents.
            """
            child_fitness = (parent1.getFitness() + parent2.getFitness()) / 2.0
            return DummyIndividual(child_fitness)
        ...
        def create_new_gen(self, population, type_choosing_parent="best", type_matching="random", crossover_type=2):
            ...
            if type_choosing_parent == "selection par rang":
                ...
                for _ in range(self.population_size // 2):
                    ...
                    cumul = 0
                    for i, w in enumerate(poids_temp):
                        cumul += w
                        if seuil <= cumul:
            ...
            # ----- Sélection par roulette (avec fitness comme poids) -----
            if type_choosing_parent == "selestion par roulette":  # note : coquille "selestion"
                parents = []
            ...
            # ----- Génération d'enfants (croisement) -----
            child = copy.deepcopy(parents)
            
            if type_matching == "random":
                # Croisement de couples aléatoires
                ...
                # On retire des individus si on dépasse la taille cible
